...
{
        //hw_uart_send(HW_UART2, "usb_cdc_cb\r\n", 12, NULL, NULL);
...
void usb_cdc_task(void *params)
{
        ...
        while (1) {
                ...
                /*
                if(mistake == 1){
                        NumBytesReceived = USBD_CDC_Receive(hInst, sector_buffer, sizeof(sector_buffer), 10);
                }else{
                ...
                */
                ...
                if (NumBytesReceived > 0) {
                        ...
                        U32 aa[2];
                        U8 ab[2],ac[2],ad[2];
                        aa[0]=pLineCoding.DTERate;
                        ab[0]=pLineCoding.CharFormat;
                        ac[0]=pLineCoding.DataBits;
                        ad[0]=pLineCoding.ParityType;
                        hw_uart_send(HW_UART2, aa, 5, NULL, NULL);
                        hw_uart_send(HW_UART2, "\r\n", 2, NULL, NULL);
                        hw_uart_send(HW_UART2, ab, 5, NULL, NULL);
                        hw_uart_send(HW_UART2, "\r\n", 2, NULL, NULL);
                        hw_uart_send(HW_UART2, ac, 5, NULL, NULL);
                        hw_uart_send(HW_UART2, "\r\n", 2, NULL, NULL);
                        hw_uart_send(HW_UART2, ad, 5, NULL, NULL);
                        hw_uart_send(HW_UART2, "\r\n", 2, NULL, NULL);
                        */
                        uint16_t n=0;
                        int cur_len;
...
void usb_cdc_stop()
{
        //hw_uart_send(HW_UART2, "usb_cdc_stop\r\n", 14, NULL, NULL);
        USBD_UnregisterSCHook(&UsbpHook);
        USBD_DeInit();
...
{
        //hw_uart_send(HW_UART2, "usb_start_cb\r\n", 14, NULL, NULL);
        if (run_usb_task == 0) {
                pm_stay_alive();
                run_usb_task = 1;
...
{
        hw_uart_send(HW_UART2, "usb_detach_cb\r\n", 15, NULL, NULL);
        if (run_usb_task == 1) {
                hw_usb_bus_detach();
                usb_cdc_stop();
...
void           USBD_CDC_SetOnLineCoding            (USB_CDC_HANDLE hInst, USB_CDC_ON_SET_LINE_CODING * pf)
{
        ...
        hw_uart_send(HW_UART2, "USBD_CDC_SetOnLineCoding\r\n", 26, NULL, NULL);
}
*/
