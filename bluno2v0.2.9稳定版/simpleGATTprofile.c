...
static void send_FE(uint8_t *pbuf, uint8_t len){
        uint8_t my_buf[24];
        uint8_t cs = 0;
        my_buf[0] = 0x55;
        my_buf[1] = 0xaa;
        ...
        hw_uart_send(HW_UART2, my_buf, len+4, NULL, NULL);
}
void serial_blewrite(uint8_t *pbuf, uint8_t len){
        ...
        if(si.sys_cfg.networkcmode == 'o'){
                if(si.sys_cfg.curr_role == ROLE_CENTRAL){
                        ...
                        }else if(bluno2 == false){
                                ble_gattc_write_no_resp(0,37,false,len,pbuf);///bluno一代采用的write_no_resp();为了兼容bluno1代的产品
                        }
                }else{
                        ble_gatts_send_event(myconn_idx, spG->DFBV1, GATT_EVENT_NOTIFICATION, len, pbuf);
                }
        }else if(si.sys_cfg.networkcmode == 's'){//在星型网络下
                if(pbuf[0]==0xFD){//发送组网信息
                        ble_gatts_send_event(pbuf[2], spG->DFBV3, GATT_EVENT_NOTIFICATION, len, pbuf);//代改
                }else if(len>4){
                        if(si.sys_cfg.curr_role == ROLE_CENTRAL){
                                if(pbuf[0] < 0xFA){
#if usblog
                                        if(open_usb == true){
                                                myserialusb("serial_blewrite\r\n",17);
                                                myserialusb("send->",6);
                                                if(pbuf[0] == 0x00){
                                                        myserialusb("0",1);
                                                }else if(pbuf[0] == 0x01){
...
#endif
                                        ble_gatts_send_event(pbuf[0], spG->DFBV3, GATT_EVENT_NOTIFICATION, len, pbuf);
                                }else{
                                        if(pbuf[0] == 0xFF){//如果打包数据第一个为0xFF则为广播数据，即发送给所有连接的设备
                                                broadcast_data(pbuf,len);//用来给所有连接上的设备发送数据
                                        }else if(pbuf[0] == 0xFA){
                                                broadcast_net_message(pbuf,len);
                                                //用来给所有连接的设备发送打包好的组网信息。
                                        }
                                        /*
                                        if(pbuf[0] == 0xFF){//如果打包数据第一个为0xFF则为广播数据，即发送给所有连接的设备
                                                broadcast_data(pbuf,len);//用来给所有连接上的设备发送数据
                                        }else if(pbuf[0] == 0xFE){//发送给连接我的上级设备//主机收到0xFE就是把数据给自己。
                                                send_FE(pbuf,len);
                                        }else{
                                                broadcast_net_message(pbuf,len);
                                                //用来给所有连接的设备发送打包好的组网信息。
...
#endif
                                ble_gatts_send_event(0, spG->DFBV3, GATT_EVENT_NOTIFICATION, len, pbuf);
                        }
                }
#if 0
                if(pbuf[0]==0xFD){//发送组网信息
                        ...
                        if(si.sys_cfg.curr_role == ROLE_CENTRAL){
                                if(pbuf[0] == 0xFF || pbuf[0] == 0xFE || pbuf[0] == 0xFA){
                                        if(pbuf[0] == 0xFF){//如果打包数据第一个为0xFF则为广播数据，即发送给所有连接的设备
                                                broadcast_data(pbuf,len);//用来给所有连接上的设备发送数据
                                        }else if(pbuf[0] == 0xFE){//发送给连接我的上级设备//主机收到0xFE就是把数据给自己。
                                                uint8_t my_buf[24];
                                ...
                                }else{//发送给指定设备//数据不作解析，只是根据数据的第一项，来判断发送给哪个从机，从机收到数据后再作解析，因为从机需要知道是这条消息的属性信息。即来自哪个从机及是多少条信息等。
                                        ble_gatts_send_event(pbuf[0], spG->DFBV3, GATT_EVENT_NOTIFICATION, len, pbuf);
                                }
                        }else if(si.sys_cfg.curr_role == ROLE_PERIPHERAL){//如果是从机，则将数据发送给主机。
                ...
                if(len>4){
                        ...
                        if(si.sys_cfg.curr_role == ROLE_CENTRAL){
                                if(aims_id == 0xFF){//如果打包数据第一个为0xFF则为广播数据，即发送给所有连接的设备
                                        int num = 0;
                                        broadcast_data(pbuf,len);//用来给所有连接上的设备发送数据
                                }else if(aims_id == 0xFE){//发送给连接我的上级设备//主机收到0xFE就是把数据给自己。
                                        uint8_t my_buf[24];
                                        uint8_t cs = 0;
                                        my_buf[0] = 0x55;
                                        my_buf[1] = 0xaa;
                                        ...
                                        hw_uart_send(HW_UART2, my_buf, len+4, NULL, NULL);
                                        //ble_gatts_send_event(0, spG->DFBV3, GATT_EVENT_NOTIFICATION, len, pbuf);
                                }else if(aims_id == 0xFA){
                                        broadcast_net_message(pbuf,len);
                                        //用来给所有连接的设备发送打包好的组网信息。
                                }else{//发送给指定设备//数据不作解析，只是根据数据的第一项，来判断发送给哪个从机，从机收到数据后再作解析，因为从机需要知道是这条消息的属性信息。即来自哪个从机及是多少条信息等。
                                        ble_gatts_send_event(aims_id, spG->DFBV3, GATT_EVENT_NOTIFICATION, len, pbuf);
                                }
                        }else if(si.sys_cfg.curr_role == ROLE_PERIPHERAL){//如果是从机，则将数据发送给主机。
                                ble_gatts_send_event(myconn_idx, spG->DFBV3, GATT_EVENT_NOTIFICATION, len, pbuf);
                        }
...
static void handle_write_req(ble_service_t *svc, const ble_evt_gatts_write_req_t *evt)
{
        ...
        if(evt->handle == spG->DFBV1){
                ...
                if(evt->value[0] == 0xFD && evt->value[1] == 0xFD){//查询组网信息
#if wx_UART_DEBUG
                        printf("查询组网信息\r\n");
#endif
                        send_network(evt->value,evt->length);
                }else{

                        if(evt->value[0] == 0xFE){//数据包头为0xFE时，则说明数据就是给自己的。
                                ...
                                //serial_blewrite(evt->value,evt->length);
                        }
                        //hw_uart_send(HW_UART2, evt->value, evt->length, NULL, NULL);
                }
                //hw_uart_send(HW_UART2, evt->value, evt->length, NULL, NULL);
