...
#define dbg_low_print
#ifdef dg_LOWPOWER
extern U32 baudrate_num;
extern uint16_t low_time;
extern void Open_ble_data_timer();
extern void Open_rx_tx_timer();
#endif
...
static void handle_write_req(ble_service_t *svc, const ble_evt_gatts_write_req_t *evt)
{
        ...
        if(evt->handle == spG->DFBV1){
                if(open_usb == true){
                        ...
                        myserialusb((uint8_t *)evt->value, evt->length);
                }else{
                        Tx_blink_hight();
                        hw_uart_send(HW_UART2, evt->value, evt->length, NULL, NULL);
                }
