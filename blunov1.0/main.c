...
extern void Serial_Reinit1();
extern void Open_ble_data_timer();
extern void Open_rx_tx_timer();
extern void Open_link_timer();
extern void Serial_App_Resume();
#endif
...
{
        //hw_uart_send(HW_UART2, "boot_application\r\n", 18, NULL, NULL);
        nvms_t update_part;
        nvms_t exec_part;
        nvms_t header_part;
        ...
        //printf("Starting image at0x%X\r\n",(unsigned int) image_address);
        //printf("reset vector 0x%X.\r\n",(unsigned int) image_address[1]);
        TRACE("Starting image at111 0x%X, reset vector 0x%X.\r\n", (unsigned int) image_address,
                                                                        (unsigned int) image_address[1]);
        if (!image_sanity_check(image_address)) {
                //hw_uart_send(HW_UART2, "Current executable insane, starting SUOTA.\r\n", 44, NULL, NULL);
...
static void test_otp_mac_flash(uint32_t first,uint32_t next){
        ...
        for(int i = 0; i<2; i++){///后两位
                otp_mac[1-i] = next_4[i];
        }
        for(int i = 0;i<6;i++){
                si.sys_cfg.my_mac[i] = otp_mac[5-i];
        };
...
#if defined ENCRYPTION
        uint32_t my_otp_mac[8];
        hw_otpc_init();

        hw_otpc_set_speed(5);
        hw_otpc_dma_read(my_otp_mac,7499,0,8,false);

        test_otp_mac_flash(my_otp_mac[0],my_otp_mac[1]);
