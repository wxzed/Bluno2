...
void Open_ble_data_timer(){
        if(ble_connect == 1){
                if(!OS_TIMER_IS_ACTIVE(wx_timer)){//如果在低功耗模式下，发送time没有开启的话,则开启time
...
void Open_rx_tx_timer(){
        if(!OS_TIMER_IS_ACTIVE(rx_tx_blink)){//如果在低功耗模式下，发送time没有开启的话,则开启time
...
void Open_link_timer(){
        if(!OS_TIMER_IS_ACTIVE(link_timer)){//如果在低功耗模式下，发送time没有开启的话,则开启time
...
static void handle_evt_gap_connected(ble_evt_gap_connected_t *evt)
{
        ...
        */
        if(conn_role == ROLE_CENTRAL){
                ...
                if(conn_net_work == 'o'){                                                 /*当组网方式关闭时即为一对一连接时，连接上一个设备则停止搜索。*/
                        connect_full = true;
                        ble_gap_scan_stop();
...
static void handle_evt_gattc_notification(ble_evt_gattc_notification_t *evt)
{
        ...
        if(conn_role == ROLE_CENTRAL){//主机收到notif数据时
                myconnid = 0xFE;
                if(conn_net_work == 'o'){//当模式不是组网状态时
                        if(open_usb == false){
                ...
                }else if(conn_net_work == 's'){//当主机模式为星型网络时
#if OLED_PRINT
                ...
                if(conn_net_work == 's'){//从机在星型网络下数据的流向。
#if 1
        ...
        if(conn_net_work == 's'){
                ...
                if(hw_gpio_get_pin_status(HW_GPIO_PORT_1,HW_GPIO_PIN_7) == false){//准备录入白名单
                        if((evt->rssi)>220){//说明设备离的很近
                                //PAIR_LED_Low();
                                if(getserverid(evt) == 0xdfb0 && getdfnode(evt) == 'n'){///'dfn'就是说主机只有看见广播里的信息这几位是dfn时才会主动连接。
                                        for(int j = 0; j<10; j++){
                ...
                if(hw_gpio_get_pin_status(HW_GPIO_PORT_1,HW_GPIO_PIN_7) == false){//录入设备
                        if((evt->rssi)>220){
                                ...
                                if(getserverid(evt) == 0xdfb0 && getdfnode(evt) == 'n'){///'dfn'就是说主机只有看见广播里的信息这几位是dfn时才会主动连接。
                                        ...
                                        now_addr = evt->address.addr[5];
                                        ble_gap_connect(&equipment_addr, &equipment_cp);
                                }else if(getserverid(evt) == 0xdfb0 && getdfnode(evt) != 'n' && getdfnode(evt) != 'r'){////////////用于连接bluno1代(临时用的)
                                        bluno2 = false;
                                        ble_gap_scan_stop();
                                        OS_DELAY(5);
                                        const bd_address_t equipment_addr ={
                        ...
                        if(Allow_connection == true){
                                if(getserverid(evt) == 0xdfb0 && getdfnode(evt) == 'n'){///'dfn'就是说主机只有看见广播里的信息这几位是dfn时才会主动连接。
                                        ...
                                        now_addr = evt->address.addr[5];
                                        ble_gap_connect(&equipment_addr, &equipment_cp);
                                }else if(getserverid(evt) == 0xdfb0 && getdfnode(evt) != 'n' && getdfnode(evt) != 'r'){////////////用于连接bluno1代
                                        bluno2 = false;
                                        ble_gap_scan_stop();
                                        OS_DELAY(5);
                                        const bd_address_t equipment_addr ={
