...
void usb_cdc_task(void *params)
{
        ...
        //wxc = OS_MALLOC(sizeof(*wxc));
        //memset(wxc, 0, sizeof(*wxc));
        //wxc->buffer = OS_MALLOC(sizeof(uint8_t) * USB_MAX_PACKET_SIZE);
        evt->conn_idx = 0;
        evt->handle = 32;
        evt->offset = 0;
        evt->length = 0;
                ...
                if (NumBytesReceived > 0) {
                        ...
                        if(getFwversionflag == 1 && startUpgradeflag == 1 && dataconfflag ==1){
                                ...
                                if((sector_buffer[0] == 0x55) && (sector_buffer[4] == 0x04) && (NumBytesReceived ==(sector_buffer[3]*16*16+sector_buffer[2]+5))){//如果收到的数据包和发过的整个包的数据相等
                                        ...
                                        //hw_uart_send(HW_UART2, sector_buffer+9, NumBytesReceived-9, NULL, NULL);
                                        dst_offset = 0;
                        ...
                        if(getFwversionflag == 0 && startUpgradeflag == 0 && dataconfflag == 0){
                                if((sector_buffer[0] == 0x55) && (sector_buffer[4] == 0x01)){
                                        getFwversionflag = 1;
                         ...
                         while(NumBytesReceived){
                                 NumBytesReceived--;
                                 uint8_t wx_buf=sector_buffer[num];
                                 head = AppendNode(head,wx_buf);
                                 num++;
                         ...
                         do{
                            ...
                            //head = AppendNode(head,sector_buffer+n,cur_len);
                            head = sappEnqueue(head,sector_buffer+n,cur_len);
                            n  += cur_len;
                            NumBytesReceived -=cur_len;
                         }while(NumBytesReceived != 0);
                        // head = AppendNode(head,sector_buffer,NumBytesReceived);
                         DispLink(head);
                         DeleteMemory(head);
                         head = NULL;
#elif 0
                         do{
                            cur_len = NumBytesReceived > MAX_SPP_PACKET_LEN ? MAX_SPP_PACKET_LEN : NumBytesReceived;
                            blesappEnqueue(sector_buffer+n,cur_len);
                            n  += cur_len;
                            NumBytesReceived -=cur_len;
                         }while(NumBytesReceived != 0);
                         //wx_bleshow();
                         ...
                         do{
                            cur_len = NumBytesReceived > MAX_SPP_PACKET_LEN ? MAX_SPP_PACKET_LEN : NumBytesReceived;
                            blehead = bleAppendNode(blehead,sector_buffer+n,cur_len);
                            n  += cur_len;
                            NumBytesReceived -=cur_len;
                         }while(NumBytesReceived != 0);
                         bleDispLink(blehead);
                         bleDeleteMemory(blehead);
                         blehead = NULL;
                         ...
                         /*测试usb发送AT指令*/
                         if(si.at_mode == 1){
                                 atParser(myCOM, sector_buffer, NumBytesReceived);
                         }else if(NumBytesReceived == 3){
                                 judgmentAT(sector_buffer,NumBytesReceived);
                         }else if(ble_connect == 1){
                                 do{
                                     cur_len = NumBytesReceived > MAX_SPP_PACKET_LEN ? MAX_SPP_PACKET_LEN : NumBytesReceived;
                                     bleAppendNode(sector_buffer+n,cur_len);
                                     n  += cur_len;
                                     NumBytesReceived -=cur_len;
                                 }while(NumBytesReceived != 0);
                                 wx_write_notif();
                         }else if(si.usb_serial_open == 1){
                                 hw_uart_send(HW_UART2, sector_buffer, NumBytesReceived, NULL, NULL);
                         }else{
...
*/
                        ...
                        //printf("NumBytesReceived=%d\r\n",NumBytesReceived);
                        //evt->length = NumBytesReceived;
                        //memcpy(wxc->buffer, sector_buffer, NumBytesReceived);
                        //wx_NumBytesReceived += NumBytesReceived;
                        //evt->offset += NumBytesReceived;
                        //wxc->buffer_len += NumBytesReceived;
                        //n +=NumBytesReceived;
...
void usb_cdc_start()
{
        ...
        status = OS_TASK_CREATE("UsbCdcTask",   /* The text name assigned to the task, for
                        ...
                        1000,                    /* The number of bytes to allocate to the
                                                   stack of the task. */
