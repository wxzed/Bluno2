...
uint8_t judgmentAT(uint8 *buf,uint16_t len){
        if(buf[0] == '+' && buf[1] == '+' && buf[2] == '+'){
                si.at_mode = 1;
                hw_uart_send(HW_UART2,"Enter AT Mode\r\n",strlen("Enter AT Mode\r\n"),NULL,NULL);
                return true;
        }else{
                return false;
        }
...
uint8 doMac(enum data_src src,uint8 *atCmd)
{
    ...
    if(*p != '='){
        return FAILURE;
            ...
            uint8 mac_addr_ascii[B_ADDR_LEN] = defaultBLE_STATIC_ADDRESS;
