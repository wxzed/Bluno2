...
static void Decode(UINT4 *, unsigned char const *, unsigned int);
...
void MD5Init (MD5_CTX *context)
{
  ...
  context->state[0] = 0x67452304;
  context->state[1] = 0xeecdab89;
  context->state[2] = 0x9abadcfe;
  context->state[3] = 0x10327476;
}
 ...
 */
  for(i = partLen; i + 63 < inputLen; i += 64 )/*把i+63<inputlen改为i+64<=inputlen更容易理解*/
   MD5Transform(context->state, (unsigned char const*)&input[i]);
...
static void MD5Transform (UINT4 state[4], unsigned char const block[64])
{
 ...
 /* Round 1 */
 FF(a, b, c, d, x[ 0], S11, 0xd76aa470); /* 1 */
 FF(d, a, b, c, x[ 1], S12, 0xe8c7b756); /* 2 */
 FF(c, d, a, b, x[ 2], S13, 0x2420e0db); /* 3 */
 FF(b, c, d, a, x[ 3], S14, 0xc1bdfeee); /* 4 */
 FF(a, b, c, d, x[ 4], S11, 0xf57cafaf); /* 5 */
 FF(d, a, b, c, x[ 5], S12, 0x4737c62a); /* 6 */
 FF(c, d, a, b, x[ 6], S13, 0xa8304613); /* 7 */
 FF(b, c, d, a, x[ 7], S14, 0xfd4c9501); /* 8 */
 FF(a, b, c, d, x[ 8], S11, 0x698092d8); /* 9 */
 FF(d, a, b, c, x[ 9], S12, 0x8b44f7af); /* 10 */
 FF(c, d, a, b, x[10], S13, 0xffff50b1); /* 11 */
 FF(b, c, d, a, x[11], S14, 0x895cd1be); /* 12 */
 FF(a, b, c, d, x[12], S11, 0x6b900122); /* 13 */
 FF(d, a, b, c, x[13], S12, 0xfd987093); /* 14 */
 FF(c, d, a, b, x[14], S13, 0xa639438e); /* 15 */
 FF(b, c, d, a, x[15], S14, 0x49840821); /* 16 */

 /* Round 2 */
 GG(a, b, c, d, x[ 1], S21, 0xf21e2562); /* 17 */
 GG(d, a, b, c, x[ 6], S22, 0xc340b340); /* 18 */
 GG(c, d, a, b, x[11], S23, 0x265e6a51); /* 19 */
 GG(b, c, d, a, x[ 0], S24, 0xe976c7aa); /* 20 */
 GG(a, b, c, d, x[ 5], S21, 0xd628105d); /* 21 */
 GG(d, a, b, c, x[10], S22, 0x2a441453); /* 22 */
 ...
 /* Round 4 */
  ...
  II(c, d, a, b, x[14], S43, 0xab9b23a7); /* 51 */
  II(b, c, d, a, x[ 5], S44, 0xfc93ae39); /* 52 */
  II(a, b, c, d, x[12], S41, 0x655f59c3); /* 53 */
  II(d, a, b, c, x[ 3], S42, 0x8f0ccc92); /* 54 */
  II(c, d, a, b, x[10], S43, 0xffef347d); /* 55 */
  II(b, c, d, a, x[ 1], S44, 0x85845dd1); /* 56 */
  II(a, b, c, d, x[ 8], S41, 0x6fa82e4f); /* 57 */
  II(d, a, b, c, x[15], S42, 0xfe2ce6e0); /* 58 */
  II(c, d, a, b, x[ 6], S43, 0xa3010314); /* 59 */
  II(b, c, d, a, x[13], S44, 0x4e081161); /* 60 */
  II(a, b, c, d, x[ 4], S41, 0xf7237e82); /* 61 */
  II(d, a, b, c, x[11], S42, 0xb43af235); /* 62 */
  II(c, d, a, b, x[ 2], S43, 0x2ad752bb); /* 63 */
  II(b, c, d, a, x[ 9], S44, 0xeb86d791); /* 64 */
